...
const userSchema = z.object({
  ...
  smtpHost: z.string().min(1, "SMTP Host is required").default("mail.dtradesinternational.in"),
...
export default function Users() {
  ...
  // SMTP synchronisation logic removed
  ...
  const openEdit = (u: any) => {
    ...
    form.reset({
      ...
      smtpHost: u.smtpHost || "mail.dtradesinternational.in",
  ...
  return (
    <AppLayout>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <Table>
                ...
                <TableBody>
                  {users?.map((u: any) => (
                    <TableRow key={u.id} className="hover:bg-primary/[0.02] transition-all duration-150">
                      ...
                      <TableCell>
                        {u.role === "admin" ? (
                          <Badge
                            className="capitalize text-xs text-white border-0"
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <span className={`status-dot ${u.isActive ? 'status-dot-success' : ''}`} />
                          <span className={`text-xs font-medium ${u.isActive ? 'text-foreground' : 'text-muted-foreground'}`}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-primary/10 hover:text-primary transition-colors" onClick={() => openEdit(u)}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-3xl rounded-2xl border-t-4 border-t-primary">
          <DialogHeader>
            <DialogTitle className="text-lg font-semibold text-foreground">{editingId ? "Edit User" : "Create User"}</DialogTitle>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 px-1 pb-1 pt-2">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <FormField control={form.control} name="dailyLimit" render={({ field }) => (
                    <FormItem><FormLabel>Daily Send Limit</FormLabel><FormControl><Input type="number" {...field} placeholder="500" /></FormControl><FormMessage /></FormItem>
                  )} />
                ...
                <div className="space-y-4 border-t md:border-t-0 md:border-l md:pl-6 pt-4 md:pt-0">
                  <h4 className="text-sm font-semibold text-foreground pb-2 border-b">SMTP Outbound Configuration</h4>
                  <FormField control={form.control} name="smtpHost" render={({ field }) => (
                    <FormItem>
                      <FormLabel>SMTP Host</FormLabel>
                      <FormControl><Input {...field} placeholder="mail.dtradesinternational.in" /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
